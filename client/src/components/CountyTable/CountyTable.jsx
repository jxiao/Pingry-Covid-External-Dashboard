...
const CountyTable = (function (props) {
  ...
  if (!props.data || props.data.length === 0 || !props.countyData || props.countyData.length === 0) {
    return null; 
  }
  ...
  return (
      ...
      <Col md={12} xl={8} className={styles.table}>
          <Card className={styles.noBottom}>
              <Card.Header>
                  <Card.Title as='h5' className={styles.large}>Breakdown by NJ Counties</Card.Title>
                  <p className={styles.paragraph}>Highlighted rows indicate heavy Pingry residence</p>
              </Card.Header>
                  ...
                  <table className={cx("table", styles.noBottom, styles.tableFixed)}>
                    <thead className={styles.theadTable}>
                      ...
                      <th  className={cx(styles.thTable, styles.one)}><ReactFitText compressor={0}><p>County</p></ReactFitText></th>
                        ...
                        <th  className={cx(styles.thTable, styles.five)}><ReactFitText compressor={0}><p data-tip={"Average # of new infections per infected"}>14-day Average Infection Rate</p></ReactFitText></th>
                      </tr>
                    </thead>
                    <tbody className={styles.tbodyTable}>
                      {props.data.map((county, i) => {
                        const highlight = indicesOfHighlightedCounties.includes(i);
                        const currSeven = props.countyData[i].counties[0].historicData[0].positiveCt - props.countyData[i].counties[0].historicData[6].positiveCt;
                        const prevSeven = props.countyData[i].counties[0].historicData[7].positiveCt - props.countyData[i].counties[0].historicData[13].positiveCt;

                        var infectionRate14DayWeightedAverage = 0;
                        props.fetchedCountyProjections[0].data[i].infectionRates.forEach((rate) => {infectionRate14DayWeightedAverage += rate.Rt;})
                        infectionRate14DayWeightedAverage /= 14;

                        return (<tr key={i} className={ cx(highlight ? styles.highlight : '', styles.trTable)}>
                                  <td className={styles.tdTable}>
                                      <h6 className={cx("mb-1", styles.small)}>{county.name}</h6>
                                  </td>
                                  <td className={styles.tdTable}>
                                      <h6 className={cx("text-muted", styles.small)}>{county.totalCases.toLocaleString('en-US')} cases</h6>
                                  </td>
                                  <td className={styles.tdTable}>
                                    <h6 className={cx("text-muted", styles.small)}>{currSeven < 0 ? "-" : "+"}{currSeven.toLocaleString('en-US')}</h6>
                                  </td>
                                  <td className={styles.tdTable}>
                                    <h6 className={cx("text-muted", styles.small)}>{prevSeven < 0 ? "-" : "+"}{prevSeven.toLocaleString('en-US')}</h6>
                                  </td>
                                  <td className={styles.tdTable}>
                                    <h6 className={cx("text-muted", styles.small)}>{infectionRate14DayWeightedAverage.toFixed(2)}</h6>
                                  </td>
                                </tr>);
                      })}
                    </tbody>
